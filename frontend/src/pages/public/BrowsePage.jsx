      ...
      <main className="mx-auto max-w-7xl px-6 py-10 md:px-12">
        <div className="flex flex-col justify-between gap-4 border-b border-stone-200 pb-5 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-1 text-sm text-stone-500">{isLoading ? 'Finding spaces…' : `${filteredSpaces.length} ${filteredSpaces.length === 1 ? 'space' : 'spaces'} found`}</p>
          </div>
        ...
        {isLoading ? <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{[1, 2, 3].map((item) => <div key={item} className="h-96 animate-pulse rounded-xl bg-stone-200" />)}</div> : null}
