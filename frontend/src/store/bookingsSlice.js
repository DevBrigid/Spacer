...
const localBookings = () => mockDatabase.bookings.map((booking) => ({ id: booking.id, userId: booking.user_id, client: mockDatabase.users.find((user) => user.id === booking.user_id)?.name || 'Spacer Client', space: booking.space_name, date: booking.start_time?.split('T')[0], duration: booking.duration_hours, amount: booking.total_amount, status: booking.status.charAt(0).toUpperCase() + booking.status.slice(1) }));
...
export const { setBookingDetails, addBooking, confirmBooking, resetBooking, approveBooking, rejectBooking } = bookingsSlice.actions;
